...
function _getTLDList(callback) {
  ...
  request(_mozilla_list, function (error, resp, body) {
    ...
    if (error || resp.statusCode !== 200) {
      require('fs').readFile('./tld_snapshot', function (error, data) {
        if (error) return callback(error);
        tlds = data.toString().split('\n');
        _cache = tlds;
        callback(null, tlds);
      });
    } else {
      tlds = body.match(/^([.*!]*\w[\S]*)/gm);
      _cache = tlds;
      // require('fs').writeFileSync(__dirname + '/tld_snapshot', tlds.join('\n'));
      callback(null, tlds);
    }
  });
}
...
TLDEXTRACT.extract = function (url, callback) {
  ...
  tld = _getTLDList(function (error, tlds) {
    if (error) return callback(error);
